...
The v1 API is read-only and requires an API key.
...
Test access does not require an account. A test key does not expire automatically, but it only works in the test environment and is subject to its assigned rate and monthly request limits. Production access is reviewed separately and may require a paid capacity agreement.
